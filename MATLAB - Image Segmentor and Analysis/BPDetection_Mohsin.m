function [strand_collection, points_collection, non_visited_points]  = ...
            BPDetection_Mohsin(BW_thin, branch_points, end_points, ...
                               rows, columns)
    ...
    if(~current_point_reference.visited)
        ...
        if(count == 2)
            if (~isempty(non_visited_neighbour_points) && ~current_strand_flag)
                current_strand = Strand;
                current_strand_flag = 1;
                if isempty(current_point_reference.parent)
        ...
        if count > 2
            if (current_point_reference.special_point && current_strand_flag)
                current_strand.points(end+1,:) = current_point;
                temp_branch_points = [];
                Cnp = intersect(branch_points,non_visited_neighbour_points,'stable','rows');
                for i = 1:size(neighbour_points,1)
                ...
                current_strand_flag = 0;
                if(~isempty(Cnp))
                    non_visited_points = [non_visited_points; non_visited_neighbour_points; Cnp];
                else    
                    non_visited_points = [non_visited_points; non_visited_neighbour_points];
                end
