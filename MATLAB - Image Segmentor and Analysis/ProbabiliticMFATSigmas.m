function out = ProbabiliticMFATSigmas(I,options)
...
sigmas      = [1e-5 options.sigmas1 : options.sigmasScaleRatio : options.sigmas2];        
...
for j = 1:length(sigmas)
    ...
    %% (1) Eigen-values
    [~, Lambda2] = imageEigenvalues(I,sigmas(j), filterSizeArray(j), spacing,whiteondark); 
...
function [Lambda1, Lambda2] = imageEigenvalues(I,sigma, filterSize, spacing,whiteondark)
% calculates the two eigenvalues for each voxel in a volume

% Calculate the 2D hessian
[Hxx, Hyy, Hxy] = Hessian2D(I,sigma, filterSize, spacing);
...
function [Dxx, Dyy, Dxy] = Hessian2D(I,Sigma,filterSize,spacing)
...
if(Sigma>0)
    F = imgaussfilt(I,Sigma, 'FilterSize', filterSize, 'FilterDomain', 'spatial');
else
    F = I;
end
